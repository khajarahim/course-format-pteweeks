...
/**
 * Specialised restore for format_pteweeks
 *
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class restore_format_pteweeks_plugin extends restore_format_plugin {
    ...
    protected function update_automatic_end_date($enddate) {
        ...
        if (empty($enddate)) {
            ...
            format_pteweeks::update_end_date($this->step->get_task()->get_courseid());
        } else {
            $update->value = 0;
            $DB->update_record('course_format_options', $update);
